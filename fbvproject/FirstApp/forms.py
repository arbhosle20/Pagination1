...
class LaptopModelForm(forms.ModelForm):
    class Meta:
        ...
        # placeholder
        widgets = {'company':forms.TextInput(attrs={'placeholder':'e.g : hp,all type'}),
                   'model_name': forms.TextInput(attrs={'placeholder': 'eg : notebook'}),
                   'ram': forms.NumberInput(attrs={'placeholder': 'eg : 2GB,4GB,8GB'}),
                   'rom':forms.NumberInput(attrs={'placeholder':'eg : 1SSD'}),
                   'processor':forms.TextInput(attrs={'placeholder':'eg : core i3'}),
                   'price':forms.NumberInput(attrs={'placeholder':'Enter the price'}),
                   'weigth':forms.NumberInput(attrs={'placeholder':'eg : in kg'})
                   }
